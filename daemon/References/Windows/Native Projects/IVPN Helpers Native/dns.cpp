...
extern "C" 
{
	EXPORT DWORD _cdecl SetDNSByIndex(const WORD interfaceIdx, const char* dnsIP, byte operation)
	{
		return wmiSetDNS(
			interfaceIdx,
			"",
	...
	EXPORT DWORD _cdecl SetDNSByMAC(const char* interfaceMAC, const char* dnsIP, byte operation)
	{
		return wmiSetDNS(
			-1,
			(interfaceMAC == NULL) ? "" : interfaceMAC,
			"",
			(dnsIP == NULL) ? "" : dnsIP,
			(Operation)operation);
	}
...
HRESULT wmiSetDNS(const int destInterfaceIndex, const std::string destInterfaceMAC, const std::string destInterfaceLocalAddr, std::string dnsIP, Operation operation)
{
	...
	if (dnsIP.empty() && (operation == Add || operation == Remove))
		return (HRESULT)0; // nothing to add or remove

	toLowerStr(&dnsIP);
	std::vector<std::string> curDnsSearchOrder;

	bool needCurDnsState = (operation == Add || operation == Remove);
	...
	if (!dnsIP.empty() && operation != Remove)
